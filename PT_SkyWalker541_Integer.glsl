/*
    PT SkyWalker541 Integer  v1.2.0
    by SkyWalker541  |  Written for NextUI
    File: PT_SkyWalker541_Integer.glsl
    ...
    v1.1.4 - SkyWalker541
         - Version bump for parity with Aspect variant (no changes to Integer)
    ...
    v1.0.2 - SkyWalker541
         - Added PT_PIXEL_BORDER parameter (0=OFF, 1=Subtle, 2=Moderate, 3=Strong)
           ...
           distance-from-center grid darkening — no extra texture samples
...
#pragma parameter PT_SYSTEM "== PT SkyWalker541 Integer v1.2.0 == System (0=Manual, 1=GB, 2=GBC, 3=GBA)" 1.0 0.0 3.0 1.0
...
// Uses a distFromCenter method tuned for clean integer/native scaling.
// For a more visible pixel grid, increase to mode 2 or 3 — at native
// scaling the grid is particularly sharp and satisfying at higher modes.
//   0 = OFF
...
// Integer pixel border constants (zfast_lcd method)
...
float resolveThreshold()
{
    ...
    if (PT_SYSTEM < 1.5)  return 0.58;
    if (PT_SYSTEM < 2.5)  return 0.65;
    return 0.42;
}
...
// -----------------------------------------------------------------------------
// Pixel border — distFromCenter method.
//
...
// Based on the zfast_lcd line weighting equation: y = a * (x^4 - b * x^6)
// -----------------------------------------------------------------------------
float pixelBorderFactor(vec2 coord)
{
    ...
    // Scale line weight and strength by mode
...
void main()
{
    ...
    if (PT_PALETTE > 0.5) {
        vec3 tint;
        if (PT_PALETTE < 1.5) {
            tint = vec3(0.651, 0.675, 0.518);
        } else if (PT_PALETTE < 2.5) {
            tint = vec3(0.737, 0.737, 0.737);
        } else {
